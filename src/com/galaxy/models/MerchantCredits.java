...
public class MerchantCredits {
	 ...
	 */
	public void createSubjectCreditMap(final Map<String, String> unitMap) throws InvalidStatementException,
			InvalidRomanException {
		...
		for (final String creditStmnt : this.creditStmnts) {
			...
			if (creditArr.length > 1) {
				...
				if (message.length() > 0) {
					message.insert(0, IConstants.MSG_3_1).append(creditStmnt).append(IConstants.MSG_3_2)
				...
				if (!MerchantUtils.isValidRoman(unitRomanVal)) {
					throw new InvalidRomanException(IConstants.MSG_4 + creditArr[0]);
				}
				...
				if (credits.isEmpty()) {
					...
					// credits provided.
					throw new InvalidStatementException(IConstants.MSG_5 + creditStmnt);
				}
				...
				// credit message.
				message.append(IConstants.MSG_6).append(creditStmnt);
			}
