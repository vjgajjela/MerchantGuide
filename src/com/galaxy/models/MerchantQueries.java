...
public class MerchantQueries {
	 ...
	 *         output.
	 */
	public String complileQueries(final Map<String, String> unitsMap, final Map<String, Double> creditsMap) {
		// result
		...
		for (final String query : this.queryStmnts) {
			...
			if (queryArr.length != 2) {
				...
				result.append(IConstants.MSG_7).append(IConstants.NEW_LINE);
				continue;
			}
			...
			if (!MerchantUtils.isValidRoman(unitRomanVal)) {
				result.append(IConstants.MSG_4).append(query).append(IConstants.NEW_LINE);
				continue;
			}
			...
			if (null == subject) {
				...
				// invalid query
				result.append(IConstants.MSG_7);
			}
